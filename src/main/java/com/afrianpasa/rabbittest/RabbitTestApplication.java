...
import com.afrianpasa.rabbittest.rabbitmq.Receiver;
...
public class RabbitTestApplication {
    ...
    public RabbitTestApplication(@Value("${app.exchange.name}") String topic,
                                 @Value("${app.queue.name}") String q,
                                 @Value("${app.routing.key}") String routing) {
        topicExchangeName = topic;
        queueName = q;
        routingKey = routing;
    }
